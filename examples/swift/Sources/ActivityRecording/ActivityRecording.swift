...
struct ActivityRecording {
    static func main() async {
        ...
        let config = selectedType.type.map { ActivityConfig(activityType: $0) }
        ...
        do {
            activity = try await service.startRecording(activityNamed: activityName, in: session, config: config)
        } catch {
        ...
        if case .analyzing(let task) = finalStatus {
            ...
            guard case .completed = analysisResult else {
                fputs("Analysis did not complete (status: \(analysisResult)).\n", stderr)
                exit(1)
            }
